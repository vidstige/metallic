...
impl SimplexNoise {
    pub fn sample(&self, point: &Vector2<f32>) -> f32 {
        const F2: f32 = 0.366_025_42;
        const G2: f32 = 0.211_324_87;

        let point = point * self.scale;
        let skew = (point.x + point.y) * F2;
        let cell = (point + Vector2::new(skew, skew)).map(|x| x.floor());
        let unskew = (cell.x + cell.y) * G2;
        let origin = cell - Vector2::new(unskew, unskew);
        ...
        let local1 = local0 - offset + Vector2::new(G2, G2);
        let local2 = local0 - Vector2::new(1.0, 1.0) + Vector2::new(2.0 * G2, 2.0 * G2);

        let n0 = contribution(&(cell), &local0);
        let n1 = contribution(&(cell + offset), &local1);
        let n2 = contribution(&(cell + Vector2::new(1.0, 1.0)), &local2);

        ((70.0 * (n0 + n1 + n2)) * self.strength).clamp(-1.0, 1.0)
    }
}

fn contribution(cell: &Vector2<f32>, local: &Vector2<f32>) -> f32 {
...
fn gradient(cell: &Vector2<f32>) -> Vector2<f32> {
